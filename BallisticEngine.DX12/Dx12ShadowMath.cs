...
// Cascade fitting for the DX12 sun shadows — mirrors the engine's ShadowMath.ComputeLightSpaceMatrix
// (frustum-slab bounding sphere + texel snap for stability) but builds the ortho projection in
// System.Numerics DX convention (z in [0,1]) instead of OpenTK's GL ortho (z in [-1,1]). Returns one
// view*proj per cascade. Math is otherwise identical to the GL path.
public static class Dx12ShadowMath {
    ...
    static Matrix4x4 Fit(Matrix4x4 camView, Matrix4x4 camProj, Vector3 lightTravelDir,
        ...
        // World-space camera frustum corners. DX NDC z is [0,1] (vs GL [-1,1]).
        Span<Vector3> corners = stackalloc Vector3[8];
        int idx = 0;
        for (int x = 0; x < 2; x++)
        for (int y = 0; y < 2; y++)
        for (int z = 0; z < 2; z++) {
            var ndc = new Vector4(x * 2f - 1f, y * 2f - 1f, z, 1f);   // z in [0,1]
            Vector4 w = Vector4.Transform(ndc, invVP);
            corners[idx++] = new Vector3(w.X, w.Y, w.Z) / w.W;
        }

        // Re-parameterize each near/far ray pair onto this cascade's slab.
        for (int c = 0; c < 8; c += 2) {
        ...
        // Texel-snap the center to stop edge shimmer as the camera moves.
