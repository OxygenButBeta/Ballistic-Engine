// Aurora V2 — minimal truthful diffuse GI (P2). One bounce, NO surface cache, NO temporal history yet.
//
...
// sky irradiance cube t5, sky prefilter cube t6 (table); UAV indirect u0 (table); AuroraConstants b0, AuroraSun
// b1; root SRVs GpuMaterials t7 / RtInstance[] t8 / Lights t9 / CardRadiance t10 / AuroraInstanceMeta t11;
...
StructuredBuffer<AuroraInstanceMeta> InstanceMeta : register(t11);   // per-instance {triOffset, clusterOffset, world}
...
// albedo the deferred pass used). The deferred pass SUPPRESSED its IBL diffuse ambient when Aurora is active
// (UseIBLDiffuse=0), so this is not double-counting — Aurora OWNS the diffuse indirect. Specular IBL + direct
...
// GTAO (t4, the AmbientOcclusion volume's output) applies at AoStrength — the Aurora RT trace already carries
