// Aurora V2 — surface-card lighting (P3). The plan's AuroraDirectOnCardsPass: light every surface "card" with
...
// in AuroraGi.hlsl wants to add as incoming radiance — no per-hit relighting.
//
// Bound: TLAS t0 (root SRV) | CardRadiance u0 (root UAV) | sky irradiance cube t1 (table) |
//        AuroraInstanceMeta[] t2 / RtInstance[] t3 / GpuMaterials t4 / Lights t5 (root SRVs) |
//        AuroraCardConstants b0 | bindless heap (ResourceDescriptorHeap[] for per-instance geo) | s0 clamp / s1 wrap.
...
StructuredBuffer<AuroraInstanceMeta> Instances   : register(t2);
...
uint InstanceForTri(uint tri) {
    ...
    [loop] while (lo <= hi) {
        uint mid = (lo + hi) >> 1;
        AuroraInstanceMeta m = Instances[mid];
...
void CSMain(uint3 dtid : SV_DispatchThreadID) {
    ...
    AuroraInstanceMeta meta = Instances[inst];
        ...
        [loop] for (uint k = 0; k < sr; k++) {
            ...
            if (q.CommittedStatus() == COMMITTED_TRIANGLE_HIT) {
                if (HistoryValid > 0.5 && BounceRays > 0.5) {
                    ...
                    AuroraInstanceMeta hm = Instances[hi];
