// Aurora V2 — Sıra 1: SCREEN-SPACE RADIANCE PROBES (the published Aurora final-gather front end).
//
// WHY: the baseline CSTrace (AuroraGi.hlsl) shoots a few cosine rays from EVERY full-res G-buffer pixel — ~2M
// trace points at 1080p, the dominant Aurora cost (~3.2ms measured on Bistro interior). Aurora instead places a
...
//                the EXACT AuroraGi hierarchy (screen-trace → HW RT → card sample → sky → distance falloff), write
...
//
// Bindings mirror AuroraGi.CSTrace so the ray hierarchy is shared code. Octahedral mapping: standard
...
// a pixel whose normal differs from the probe normal can still read a plausible direction (Aurora stores the full
...
StructuredBuffer<AuroraInstanceMeta> InstanceMeta : register(t11);
...
// Resolve one ray's incoming radiance with the shared AuroraGi hierarchy. `falloffApply` lets the integrate path
...
void CSPlace(uint3 dtid : SV_DispatchThreadID) {
    ...
    // Scan the tile for the valid pixel nearest the tile center (Aurora places probes on real geometry, not a
...
void CSProbeTrace(uint3 dtid : SV_DispatchThreadID) {
    ...
    // stores the FULL SPHERE of incoming radiance (Aurora screen probes are full-sphere): the integrate then picks
    ...
    // frame's atlas → many effective rays per probe at no extra trace cost (the published Aurora screen-probe
