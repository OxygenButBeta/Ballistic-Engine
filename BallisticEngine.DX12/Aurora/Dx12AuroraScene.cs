...
// Aurora V2 — the SCENE SUBSTRATE for the new GI/reflection stack (plan §"Render Architecture" item 1).
//
// This is NOT a pass and runs NO shading. It owns the durable scene representation every Aurora pass reads:
//   - the shared BLAS/TLAS (Dx12SceneAS) and per-instance bindless geometry/material SRVs (Dx12RtGeometry),
//     both reached through the shared DXR holder (ctx.Dxr) — Aurora does NOT build its own AS, it reuses the
...
//
// Gated behind BALLISTIC_DX12_AURORA: default-off = nothing allocated, byte-identical to a no-Aurora frame.
public sealed class Dx12AuroraScene : IDisposable
{
    readonly Dx12Device dev;

    // ---- per-instance meta the card-lighting compute reads (matches HLSL AuroraInstanceMeta) ----
    ...
    ID3D12Resource instanceMeta;        // AuroraInstanceMeta[] — root SRV, indexed by instance
    ...
    // hits (so the cache converges to full multi-bounce GI over a few frames — the Aurora radiance-cache trick).
    ...
    // Each record (cluster) carries a world-space card plane (Dx12AuroraCluster.ClusterCard transformed per
    ...
    // BALLISTIC_DX12_AURORA_MESHCARDS; when off, only the legacy 1-value-per-record path allocates (byte-identical).
    [StructLayout(LayoutKind.Sequential)]
    ...
    // off). Mesh-cards arm it to e.g. 4 → 16 texels/record. Env BALLISTIC_DX12_AURORA_MESHCARD_DIM overrides.
    ...
    public Dx12AuroraScene(Dx12Device device) { dev = device; }
    ...
    public bool Ensure(Dx12FrameContext ctx)
    {
        ...
        if (!ctx.Dxr.CheckAvailable("Aurora"))
            return false;

        bool prof = Environment.GetEnvironmentVariable("BALLISTIC_DX12_AURORA_PROFILE") == "1";
        ...
        else
        {
            ...
            // was the entire Aurora CPU cost (~17ms on Bistro exterior) — it ran EVERY frame, rebuilding the
        ...
        if (!loggedThisStamp)
        {
            ...
            string line = $"[Aurora] scene: objects={InstanceCount} tris={TotalTriangles} records(clusters)={RecordCount} " +
    ...
    unsafe void Rebuild(Dx12SceneAS sceneAS, Dx12RtGeometry rtGeo)
    {
        ...
        AuroraInstanceMeta[] meta = BuildMetaArray(sceneAS, out int total, out int records, out uint[] triCluster, out uint[] clusTri, out GpuClusterCard[] cards);
        TotalTriangles = total;
        RecordCount = records;

        // Sıra 5: mesh-card texel grid. Armed by BALLISTIC_DX12_AURORA_MESHCARDS=1 → TexelDim N (default 4, 16
        ...
        bool meshCards = Environment.GetEnvironmentVariable("BALLISTIC_DX12_AURORA_MESHCARDS") == "1";
        TexelDim = meshCards ? Math.Clamp((int)EnvF("BALLISTIC_DX12_AURORA_MESHCARD_DIM", 4f), 1, 8) : 1;
        ...
        instanceMeta = n > 0 ? dev.CreateUavBuffer<AuroraInstanceMeta>(meta, ResourceStates.GenericRead) : null;
    ...
    unsafe void RefreshTransforms(Dx12SceneAS sceneAS)
    {
        ...
        // BALLISTIC_DX12_AURORA_PARTIAL_REFRESH=0 reverts to the full rebuild (needTriMaps=true) for A/B.
        bool partial = Environment.GetEnvironmentVariable("BALLISTIC_DX12_AURORA_PARTIAL_REFRESH") != "0";
        AuroraInstanceMeta[] meta = BuildMetaArray(sceneAS, out _, out _, out _, out _, out GpuClusterCard[] cards,
                                                  needTriMaps: !partial);
        ...
        dev.DeferredRelease(instanceMeta);
        instanceMeta = dev.CreateUavBuffer<AuroraInstanceMeta>(meta, ResourceStates.GenericRead);
    ...
    AuroraInstanceMeta[] BuildMetaArray(Dx12SceneAS sceneAS, out int total, out int records, out uint[] triCluster,
                                       out uint[] clusterTri, out GpuClusterCard[] cards, bool needTriMaps = true)
    {
        int n = sceneAS.InstanceCount;
        var meta = new AuroraInstanceMeta[Math.Max(n, 1)];
        int totalTris = 0;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            var mc = Dx12AuroraCluster.Cluster(sceneAS.InstanceMesh(i));
            // PP3: the triangle→cluster + cluster→tri maps are topology-only (no world matrix) — skip when the
            // caller (RefreshTransforms) doesn't need them. The clustering itself (Dx12AuroraCluster.Cluster) is
