...
// Aurora V2 #2A — greedy per-mesh triangle CLUSTERING for the surface-cache "RadianceCache". This is the cheap,
// parameterization-free stand-in for Unreal Aurora's mesh CARDS: instead of one radiance record per triangle
...
//
// Why this stays Aurora-correct for diffuse GI: diffuse indirect is LOW-FREQUENCY, so a cluster's single averaged
...
//
// The clustering is per-MESH (cached by mesh, since one mesh can have many instances) and runs only on a Aurora
...
internal static class Dx12AuroraCluster
{
    ...
    // BALLISTIC_DX12_AURORA_CLUSTER overrides.
    ...
    // cluster count. Cluster indices are LOCAL to the mesh; Dx12AuroraScene offsets them into a global record space.
    ...
    static int Target =>
        int.TryParse(Environment.GetEnvironmentVariable("BALLISTIC_DX12_AURORA_CLUSTER"), out int v) && v > 0
    ...
    static float SpatialSplitFraction =>
        float.TryParse(Environment.GetEnvironmentVariable("BALLISTIC_DX12_AURORA_CLUSTER_SPAN"), out float v) && v > 0
            ? v : 0.03f;
