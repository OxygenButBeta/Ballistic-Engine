...
public sealed class Dx12GiPass : IRenderPass, IDisposable {
    ...
    unsafe int FillSsgiConstants(Dx12FrameContext ctx) {
        var proj = ctx.Proj; var view = ctx.View;
        ...
        *(SsgiConstants*)ssgiCbMapped = new SsgiConstants {
            ...
            Params3 = new Vector4((ssgiHistValid && !ctx.DeterministicCapture) ? 1f : 0f, MathF.Max(pf.SsgiMaxHistory, 1f), GiIsolateOn(ctx) ? 1f : 0f, 0f),
        };
        return fi;
    }
    ...
    unsafe void DrawRtGi(Dx12FrameContext ctx) {
        ...
        if (!sceneAS.Valid) { DrawSsgi(ctx); return; }   // no geometry → fall back to SSGI
        ...
        *(RtGiConstants*)rtGiCbMapped = new RtGiConstants {
            ...
            Params = new Vector4(SsgiPreExposure(), MathF.Max(ctx.PostFX.SsgiRayLength, 0.1f), GiEmissiveEnabled(ctx) ? 1f : 0f, fi),
        };
        ...
        *(RtGiSun*)rtGiSunCbMapped = new RtGiSun {
        ...
        if (DdgiEnabled(ctx) && ctx.Dxr.Ddgi != null && ctx.Dxr.Ddgi.Allocated) {
            ...
            if (!ddgi.WarmupEnabled) {
                ...
                if (ddgiSw != null) { ddgiSw.Stop(); ctx.Stats.GpuPasses.Add(("GI:DDGI", ddgiSw.Elapsed.TotalMilliseconds)); }
            }
            ...
            if (gatherSw != null) { gatherSw.Stop(); ctx.Stats.GpuPasses.Add(("GI:DDGIGather", gatherSw.Elapsed.TotalMilliseconds)); }
            ssgiTarget.ColorToShaderResource();
    ...
    unsafe void DrawScreenProbeGather(Dx12FrameContext ctx, Matrix4x4 invVP) {
        var dev = ctx.Dev; var gbuffer = ctx.GBuffer; var ibl = ctx.Ibl;
        var gpuDriven = ctx.GpuDriven; var clusteredLights = ctx.ClusteredLights;
        ...
        if (sw != null) { sw.Stop(); ctx.Stats.GpuPasses.Add(("GI:ScreenProbe", sw.Elapsed.TotalMilliseconds)); }
    }
    ...
    public void Dispose() {
        ssgiGatherPso?.Dispose(); ssgiTemporalPso?.Dispose(); ssgiCombinePso?.Dispose();
        ssgiRootSig?.Dispose(); ssgiCb?.Dispose(); ssgiSrvVisible?.Dispose();
        ssgiTarget?.Dispose(); ssgiHistoryA?.Dispose(); ssgiHistoryB?.Dispose();
        ssgiDenoised?.Dispose(); ssgiScene?.Dispose();
        ssgiOidn?.Dispose(); ssgiOidnGpu?.Dispose();
        rtGiPso?.Dispose(); rtGiRootSig?.Dispose(); rtGiSbt?.Dispose(); rtGiCb?.Dispose(); rtGiSunCb?.Dispose();
        rtGiHeap?.Dispose();
