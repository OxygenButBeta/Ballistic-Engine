...
// directly. And the source the engine passes here is GLSL (e.g. SkyboxRenderer), which is NOT valid HLSL.
//
...
// setters are no-ops: the ONLY callers are the GL renderer internals and the (deferred) skybox pass —
// the engine core never sets uniforms by name. This is the impedance break the full-DX decision lets us
// take cleanly (no need to faithfully reimplement the GL uniform model).
