...
| RT reflections via world cache | ~1.5–2 | ✅ R2.3 MEASURED: "Hit Lighting" SHARP **re-shade-at-hit** (`DxrReflections.hlsl:274`), NOT blurry cache (DDGI cache = hit ambient only); roughness GRADIENT already = the split (≤0.6 re-shade / >0.6 diffuse-GI / miss prefilter cube); term unchanged ~2.07ms RX9070XT → R2.5 flag resolved |
...
| **★ R2.2** Two rates ✅ | On-screen → **fast** (SSGI+screen-probe every frame; few-frame latency, NOT "instant"). Off-screen → loose DDGI round-robin (1/8→1/16; few-seconds, accepted). Optional: priority-update near changed lights. | **DONE** (no code — both rates ALREADY WIRED + the gather-only branch ALREADY EXISTS; R2.1 §(B) "needs new code" was a STALE read, corrected by re-grep). On-screen budget = SSGI EMA (maxHistory=24): ≤10 fr to visual converge (~167ms@60 / ~333ms@30), 1–4 fr half/quarter (~17–67ms@60). Off-screen budget SEPARATE = DDGI round-robin 1/8: initial fill 8 fr (~133ms@60), step full-converge ~4.4s@60 ("few-s, accepted"); RATE DERIVED → 1/8 default, 1/16 tighter fallback (both inside plan's 1/8→1/16 few-s envelope), env-tunable + already logged. Gather-only = default `GiMode=RayTraced`+DDGI+ScreenProbe path (skips per-pixel `DispatchRays`, `Dx12GiPass.cs:669-674` early-return); runtime High stays `GiMode=ScreenSpace` on shipping surface for GPU-SAFETY (RayTraced headless device-remove risk), NOT a wiring gap → R3.2 default-flip-when-GPU-safe. Shipping A/B byte-identical (CornellBox `81dbf7a5667f`/ThinWall `30bc4b4368f5`/ColorOnly `55ec21c5cffb`). Full record: gi-revival-R0-baseline.md §R2.2. |
| **★ R2.3** Reflections ✅ | Measure what P8.0 actually is (cache vs re-shade) on a 2060. Build roughness-split: rough→cache (cheap), sharp→re-shade-at-hit (clamp rays). | **DONE** (no code — measure+document; the roughness split + re-shade ALREADY EXIST). P8.0 = **RE-SHADE-AT-HIT (sharp), NOT cache-read** (`DxrReflections.hlsl:274` `albedo*(sun+punctual+ambient)+emissive`; DDGI cache = hit AMBIENT only, not the reflection color; mirror ray = deterministic = sharp). Roughness split exists as a GRADIENT: ≤0.6 → trace+re-shade (1 mirror ray, clamped, MaxTraceRecursionDepth=1); >0.6 → early-return → diffuse GI handles it (no /PI double-count, `:27-28`); miss → roughness-mipped prefilter cube. DoD glossy-sharp SATISFIED by construction (static analysis — RayTraced headless device-unsafe §4); IBL fallback ONLY on reflection-ray MISS (escaped ray = outside BVH/far-field) = intentional+documented. **RT-refl term re-measured = UNCHANGED (~2.07ms RX9070XT, no path change) → R2.5 re-run flag RESOLVED.** Shipping (SSR) A/B byte-identical (CornellBox/ThinWall/ColorOnly). Full record: gi-revival-R0-baseline.md §R2.3. |
| **★ R2.4** Cascaded + cull ✅ | Finite volume (~30m near + clipmap fade); distant horizon → IBL/sky (**intentional**, document it). Culling = perf lever only. | **DONE** (no code — finite volume + leak-safe-culling ALREADY EXIST by construction, 3rd R2.x re-grep-finds-it-exists in a row). DDGI = finite **16×8×16 @ 2m ≈ 30×14×30m**, camera-centered **single clipmap cascade** (`Dx12Ddgi.cs:25/64/180-192`; a true clipmap/infinite cascade is out of scope §3). Beyond-volume → IBL/sky is INTENTIONAL: gather out-of-bounds `continue`→`sumW=0`→`E=0` (`DdgiGather.hlsl:119/162`) + DxrGi miss=0-bounce (`:158-159`) + DxrReflections miss=prefilter cube (`:222-226`). ★ GUARD MET: the GI/probe-visibility TLAS is fed the **FULL** `RuntimeSet<IStaticMeshRenderer>.ReadOnlyCollection` at EVERY call-site (`Dx12GiPass.cs:554/586` + `Dx12ReflectionsPass.cs:273/279` + `DX12HDRenderer.cs:1945`; `Dx12SceneAS.cs` has 0 cull refs; `Dx12RtGeometry.Ensure` filters only IsActive/IsRenderable) → off-screen occluders still block probe rays → no cull-induced leak by construction. Culling = raster-only perf lever (`AabbInFrustum`), architecturally separate from the TLAS. **Leak-test ThinWall PASSES with culling ON: isolate 0.000, SHA `30bc4b4368f5` byte-identical** (+CornellBox `81dbf7a5667f`/ColorOnly `55ec21c5cffb` no-regress, all EXIT=0 zero device-removal). Full record: gi-revival-R0-baseline.md §R2.4. |
| **★ R2.5** VRAM ✅ | Budget the real cost: **BLAS/TLAS acceleration structures** (not the tiny DDGI/probe buffers). Tie to preset. | **DONE** (no shipping code — measure+document). Re-grep FIRST: DDGI `GridVramBytes` (`Dx12Ddgi.cs:223-231` = 8.06 MB FIXED) + ScreenProbe `GridVramBytes` (`Dx12ScreenProbe.cs:108-115` = 6.4–14.4 MB, FSR-internal-res-driven) ALREADY summed+logged; **the DOMINANT BLAS/TLAS AS size is NOT exposed** — `Dx12SceneAS.Build()` computes `GetRaytracingAccelerationStructurePrebuildInfo`→`ResultDataMaxSizeInBytes` (`:81/:108`) inline-then-DISCARDS it = the genuinely-missing piece (NOT a 5th already-exists). Measured GPU-SAFE by **static tri-count analysis** (one BLAS/unique mesh + one TLAS/instance, stamp-cached `:25/52-54`): fixtures ~4 KB–198 KB, Bistro ~122–354 MB, scales with tri-count, built once. Per-preset GI ms (High @ FSR-Quality / Epic 3070+ / Low deferred): RX9070XT-extrapolated (compute 5–8× / RT 8–14×, R0.4) High ~6.9 ms RX9070XT → ~28–51 ms modeled-2060 @720p-FSR-Quality = fits 30fps optimistic; RT-refl = R2.3 FROZEN ~2.07 ms used directly (NO re-capture); **X UNCHANGED post-R1.0** (ScreenSpace cost unchanged, R0.4 budget STANDS). ThinWall leak-pass byte-identical run-to-run (lum 0.0001, `dab22a9ed962`). Full record: gi-revival-R0-baseline.md §R2.5. |
| **R3.1** Doors ✅ | Sub-system toggles (SSGI/DDGI/SCREENPROBE) → **debug-only, NOT deleted** (bisect tools). Off the shipping surface, not the code path. | **DONE** (no shipping code — 6th measure+document in a row; re-grep FIRST confirmed the DoD holds by construction). `BALLISTIC_DX12_.*GI` + the broader GI/SSGI/DDGI/SCREENPROBE/RT set grep over `BallisticEngine.DX12/` = ONLY env-var-read doors, NONE on a shipping surface; the central `Dx12RenderDoors` struct has **NO GI field** (Shadows/Ibl/Sky/Ssao/Bloom/AP/Fog/Volumes only); `BALLISTIC_FX_SSGI` is GONE from DX12. Every diffuse-GI door resolves `is null ? PostFX.X(volume) : env` (volume AUTHORITATIVE per R0.1 `d8ee45a7`, env = override-when-set); the rest (`GI_TIMING`/`DDGI_DEBUG`/`GI_MOTION_DUMP`/`GI_ORBIT`/`SSGI_OIDN`/`DDGI_WARMUP`/`DDGI_UPDATE_FRACTION`) are pure diagnostics/tunables. GI behavior changes via the volume's `GiMode` (+ R3.2 `GiQuality`), never an env-door default. Build 0-err; GI-isolate A/B byte-identical by construction (no code/default changed). Full inventory: gi-revival-R0-baseline.md §R3.1. |
| **R3.2** Volume ✅ | Add `GiQuality (High/Epic)` enum to the unified `GlobalIllumination` volume (re-verify P0.5-unified first); advanced knobs derive from preset. Use the inspector attribute pipeline (no new type-switch). | **DONE** — P0.5-unified re-confirmed (only `GlobalIllumination.cs` in Components/, no GiQuality pre-existing). Added `enum GiQuality {High,Epic}` ([PostProcessSettings.cs:38](../../Abstraction/Rendering/PostProcessSettings.cs#L38)) + `EnumParameter<GiQuality> giQuality=new(High)` front-door knob on the volume ([GlobalIllumination.cs:37](../../Engine/Rendering/Volumes/Components/GlobalIllumination.cs#L37), auto-discovered by reflection → renders through the existing DrawerRegistry, NO type-switch) + preset→dials in the bridge ([VolumePostProcessing.cs:99-130](../../Engine/Rendering/Volumes/VolumePostProcessing.cs#L99): High=4 slices/24 history == engine defaults; Epic=8/32). Preset DRIVES the effective dials (applied AFTER the per-dial copies); no `PostFX.GiQuality` field needed (renderer reads the resolved dials). **DoD MET: GI behavior changes ONLY via `GiMode` + `GiQuality`.** ★ byte-identical at default High: ThinWall `30bc4b4368f5` (run-to-run + == ref), CornellBox `81dbf7a5667f`, ColorOnly `55ec21c5cffb` — all == the R2.x reference SHAs, zero device-removal, ScreenSpace path. Low DEFERRED (§0/§3, enum is 2 RT tiers). Full record: gi-revival-R0-baseline.md §R3.2. |
| **R3.3** Maintainability ✅ | — | **DONE** (no code — verify-the-aggregate; re-grep/re-read each bullet against the tree, PROVISIONAL POLICY). All 4 hold: **(i)** GI controlled by exactly two front-door knobs `GiMode`+`GiQuality` ([PostProcessSettings.cs:27/39](../../Abstraction/Rendering/PostProcessSettings.cs#L27), [GlobalIllumination.cs:37/43](../../Engine/Rendering/Volumes/Components/GlobalIllumination.cs#L37); preset DRIVES the Advanced dials at [VolumePostProcessing.cs:113-125](../../Engine/Rendering/Volumes/VolumePostProcessing.cs#L113) AFTER the per-dial copies); **(ii)** zero manual bindless magic numbers — grep `16384 - `/`16384-` over DX12/ = only COMMENTS, no active-code literal; `Dx12BindlessTail.cs` is the single source (`HeapCapacity` named once, 4 bases DERIVED by cumulative subtraction, 8 compile-time asserts PASS on rebuild), all consumers (`Dx12Backend.cs:70`/`Dx12GiPass.cs:155-157`/`Dx12ReflectionsPass.cs:87`) read it; **(iii)** grep = debug doors only — every `BALLISTIC_DX12_*` GI/SSGI/DDGI/SCREENPROBE/RT door is `is null ? volume : env` override-when-set or pure diagnostic, and the central `Dx12RenderDoors` struct has **NO GI field** (Shadows/Ibl/Sky/Ssao/Bloom/AerialPersp/Fog/Volumes only); **(iv)** Part B §0 Keep/drop + "What was eliminated by measurement" record kept/dropped+why. Build 0-err (DX12 -t:Rebuild → asserts evaluated); ThinWall GI-isolate smoke `30bc4b4368f5` == R3.2 ref (byte-identical, ScreenSpace path, EXIT=0, zero device-removal). Full record: gi-revival-R0-baseline.md §R3.3. |
| **R3+** Auto-downgrade | OPTIONAL, off critical path. Wall-clock-ms preset-drop on the P7.0 gate. **SKIPPED** (this run) — the "fits" proof does NOT depend on it (§2) and the budget gate is **PERMANENTLY MODELED** (R0.4: no real 2060/3060 on hand). Auto-downgrade needs real-HW wall-clock telemetry to be meaningful (drop a preset when the measured frame ms exceeds budget); on a modeled-only seat there is no live ms to gate on. P7.0 no-RT auto-downgrade (RayTraced→ScreenSpace without HW RT) stays present + untouched. Determinism-gate (OFF under DETERMINISTIC) + forced-transition test + hysteresis (median-over-N) reopen if a physical card appears. | "Fits" proof does NOT depend on it. **DECISION: SKIPPED — plan COMPLETE through R3.3.** |
