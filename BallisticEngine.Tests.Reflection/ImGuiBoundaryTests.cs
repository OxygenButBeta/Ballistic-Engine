...
internal static class ImGuiBoundaryTests {
    ...
    static bool IsUnderGeneratedOrBin(string repoRoot, string path) {
        ...
        return p.Contains("/bin/", StringComparison.OrdinalIgnoreCase)
            ...
            // Sibling git worktrees live physically under .claude/worktrees/<name>/ inside the repo dir;
            // they are SEPARATE working copies (own branch/state) — never scan them, or their editor files
            // read as "outside BallisticEngine.Editor" false-positives when the scan runs from the main repo.
            || p.Contains("/.claude/worktrees/", StringComparison.OrdinalIgnoreCase);
    }
